<template>
    <div class="tuijian">
        <div class="tuijian-item">
            <div class="tuijian-item-div" v-for="(item,index) in liuxingList" :key="index">
...
export default {
    ...
    methods:{
        ...
        getJingpin(){
            this.$axios.get('/top/playlist?limit=33&order=hot&cat=日语').then((res)=>{
                this.liuxingList = res.data.playlists
                // console.log(this.huayuList)
            }).catch((err)=>{
                console.log(err)
            })
        },
